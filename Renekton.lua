...
do
    ...
    local Files = {
        Lua = {
            Path = SCRIPT_PATH,
            Name = "Renekton.lua",
            Url = "https://raw.githubusercontent.com/miragessee/GoSRenekton/master/Renekton.lua"
        },
        Version = {
            Path = SCRIPT_PATH,
            Name = "miragesrenekton.version",
            Url = "https://raw.githubusercontent.com/miragessee/GoSRenekton/master/miragesrenekton.version"
        }
    }
...
function QDmg()
    if myHero:GetSpellData(_Q).level == 0 then
        local level = 1
        local qdamage = (({65, 100, 135, 170, 205})[level] + 0.80 * myHero.bonusDamage)
        return qdamage
    else
...
function WDmg()
    if myHero:GetSpellData(_W).level == 0 then
        local level = 1
        local qdamage = (({5, 15, 25, 35, 45})[level] + 0.75 * myHero.totalDamage)
        return qdamage
    else
...
function EDmg()
    if myHero:GetSpellData(_E).level == 0 then
        local Dmg1 = (({40, 70, 100, 130, 160})[1] + 0.90 * myHero.bonusDamage)
        return Dmg1
    else
        local Dmg1 = (({40, 70, 100, 130, 160})[myHero:GetSpellData(_E).level] + 0.90 * myHero.bonusDamage)
        return Dmg1
    end
end
...
local HeroIcon = "https://www.mobafire.com/images/champion/icon/renekton.png"
local IgniteIcon = "http://pm1.narvii.com/5792/0ce6cda7883a814a1a1e93efa05184543982a1e4_hq.jpg"
local QIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/f/f3/Cull_the_Meek.png"
local WIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/b/b9/Ruthless_Predator.png"
local EIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/a/ad/Slice.png"
local E2Icon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/4/4c/Dice.png"
...
function Renekton:LoadMenu()
    ...
    self.RenektonMenu = MenuElement({type = MENU, id = "Renekton", name = "Mirage's Renekton", leftIcon = HeroIcon})
    
    self.RenektonMenu:MenuElement({id = "Harass", name = "Harass", type = MENU})
    ...
    self.RenektonMenu.Drawings:MenuElement({id = "DrawW", name = "Draw W Range", value = false})
    self.RenektonMenu.Drawings:MenuElement({id = "DrawE", name = "Draw E Range", value = true})
    self.RenektonMenu.Drawings:MenuElement({id = "DrawR", name = "Draw R Range", value = true})
    self.RenektonMenu.Drawings:MenuElement({id = "DrawAA", name = "Draw Killable AAs", value = false})
    self.RenektonMenu.Drawings:MenuElement({id = "DrawJng", name = "Draw Jungler Info", value = true})
    
    self.RenektonMenu:MenuElement({id = "blank", type = SPACE, name = ""})
    self.RenektonMenu:MenuElement({id = "blank", type = SPACE, name = "Script Ver: " .. Version .. " - LoL Ver: " .. LVersion .. ""})
    self.RenektonMenu:MenuElement({id = "blank", type = SPACE, name = "by " .. Author .. ""})
end
...
function Renekton:Tick()
    ...
    if self.RenektonMenu.AutoLevel.AutoLevel:Value() then
...
function Renekton:CollisionX(myHeroPos, dangerousPos, unitPos, radius)
...
function Renekton:CalculateEndPos(startPos, placementPos, unitPos, range)
    if range > 0 then
...
function Renekton:ProcessSpell(units)
    for i = 1, #units do
        local unit = units[i]
...
function Renekton:KillSteal()
    ...
    if self.RenektonMenu.KillSteal.UseT:Value() then
    ...
    if self.RenektonMenu.KillSteal.UseTH:Value() then
    ...
    if self.RenektonMenu.KillSteal.UseQ:Value() then
        if IsReady(_Q) then
            if ValidTarget(enemy, RenektonQ.range) and enemy.health < QDmg() then
                LocalControlCastSpell(HK_Q, enemy)
            end
        end
    end
    if self.RenektonMenu.KillSteal.UseW:Value() then
        if IsReady(_W) then
            if ValidTarget(enemy, RenektonW.range) and enemy.health < WDmg() then
                LocalControlCastSpell(HK_W, enemy)
            end
        end
    end
    if self.RenektonMenu.KillSteal.UseE:Value() then
        if IsReady(_E) then
            if ValidTarget(enemy, RenektonE.range) and enemy.health < EDmg() then
...
function Renekton:Draw()
    ...
    if self.RenektonMenu.Drawings.DrawQ:Value() then Draw.Circle(myHero.pos, RenektonQ.range, 1, Draw.Color(255, 0, 191, 255)) end
    if self.RenektonMenu.Drawings.DrawW:Value() then Draw.Circle(myHero.pos, RenektonW.range, 1, Draw.Color(255, 65, 105, 225)) end
    if self.RenektonMenu.Drawings.DrawE:Value() then Draw.Circle(myHero.pos, RenektonE.range, 1, Draw.Color(255, 30, 144, 255)) end
    if self.RenektonMenu.Drawings.DrawR:Value() then Draw.Circle(myHero.pos, RenektonR.range, 1, Draw.Color(255, 0, 0, 255)) end
    
    for i, enemy in pairs(GetEnemyHeroes()) do
        if self.RenektonMenu.Drawings.DrawJng:Value() then
        ...
        if self.RenektonMenu.Drawings.DrawAA:Value() then
            if ValidTarget(enemy) then
...
function Renekton:Harass()
    ...
    if self.RenektonMenu.Harass.UseQ:Value() and self.RenektonMenu.Harass.UseE:Value() and IsReady(_Q) and IsReady(_E) then
        ...
        local targetE = GOS:GetTarget(RenektonE.range, "AD")
        
        if targetE then
            if not IsImmune(targetE) and not IsUnderTurret(targetE.pos) then
                if IsReady(_E) and self.Collision == false and GetSpellEName() == "RenektonSliceAndDice" then
                    if ValidTarget(targetE, RenektonE.range) then
                        local hitChance, aimPosition = HPred:GetHitchance(myHero.pos, targetE, RenektonE.range, RenektonE.delay, RenektonE.speed, RenektonE.radius, false)
        ...
        local targetQ = GOS:GetTarget(RenektonQ.range, "AD")
        
        if targetQ then
            if not IsImmune(targetQ) then
                if self.RenektonMenu.Harass.UseQ:Value() then
                    if IsReady(_Q) then
                        if ValidTarget(targetQ, RenektonQ.range) then
        ...
        local targetW = GOS:GetTarget(RenektonW.range, "AD")
        
        if targetW then
            if not IsImmune(targetW) then
                if self.RenektonMenu.Harass.UseW:Value() then
                    if IsReady(_W) then
                        if ValidTarget(targetW, RenektonW.range) then
                            LocalControlCastSpell(HK_W, targetW)
                            --SetMovement(false)
                            --Control.SetCursorPos(targetW.pos)
    ...
    --if IsReady(_E) and GetSpellEName() == "RenektonDice" then
    --    LocalControlCastSpell(HK_E, myHero.pos * -1)
    --end
    else
    ...
    if IsReady(_E) and GetSpellEName() == "RenektonDice" and not IsReady(_Q) and not IsReady(_W) then
        LocalControlCastSpell(HK_E, myHero.pos * -1)
    end
end

function Renekton:CastE(target, EcastPos)
    if LocalGameTimer() - OnWaypoint(target).time > 0.05 and (LocalGameTimer() - OnWaypoint(target).time < 0.125 or LocalGameTimer() - OnWaypoint(target).time > 1.25) then
        if GetDistance(myHero.pos, EcastPos) <= RenektonE.range then
...
function Renekton:Combo()
    ...
    if self.RenektonMenu.Combo.UseT:Value() then
        if GetItemSlot(myHero, 3077) > 0 and ValidTarget(targetTiamat, 380) then
            if myHero:GetSpellData(GetItemSlot(myHero, 3077)).currentCd == 0 then
                Control.CastSpell(Item_HK[GetItemSlot(myHero, 3077)], targetTiamat)
            end
    ...
    if self.RenektonMenu.Combo.UseTH:Value() then
        if GetItemSlot(myHero, 3748) > 0 and ValidTarget(targetTHydra, 380) then
            if myHero:GetSpellData(GetItemSlot(myHero, 3748)).currentCd == 0 then
                Control.CastSpell(Item_HK[GetItemSlot(myHero, 3748)], targetTHydra)
            end
    ...
    if targetE then
        if not IsImmune(targetE) and not IsUnderTurret(targetE.pos) then
            if self.RenektonMenu.Combo.UseE:Value() then
    ...
    if targetQ then
        if not IsImmune(targetQ) then
            if self.RenektonMenu.Combo.UseQ:Value() then
                if IsReady(_Q) then
                    if ValidTarget(targetQ, RenektonQ.range) then
                        LocalControlCastSpell(HK_Q, targetQ)
                    end
    ...
    if targetW then
        if not IsImmune(targetW) then
            if self.RenektonMenu.Combo.UseW:Value() then
                if IsReady(_W) then
                    if ValidTarget(targetW, RenektonW.range) then
                        LocalControlCastSpell(HK_W, targetW)
                        --SetMovement(false)
            ...
            if self.RenektonMenu.Combo.UseWM:Value() then
                if IsReady(_W) then
                    if ValidTarget(targetW, RenektonW.range) and myHero.mana > 50 then
                        LocalControlCastSpell(HK_W, targetW)
                    end
                end
            end
    ...
    local targetR = GOS:GetTarget(RenektonR.range, "AD")
    
    if targetR then
        if not IsImmune(targetR) then
            if self.RenektonMenu.Combo.UseQ:Value() then
                if IsReady(_R) then
                    if ValidTarget(targetR, RenektonR.range) then
